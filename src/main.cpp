...
#include <random>
#include "Surface.h"
extern const std::string dataPath = std::filesystem::path(std::filesystem::current_path()) /= "../../../dane/MT/BIG";
extern const int NUMBEROFANGLES = 60;
extern const double LIMITOFCELLS = 1000000;
...
//  for (int i = 0; i < 10; i++) {
    ...
    while (numberOfCells <= LIMITOFCELLS) {
      ...
      std::cout << "\033[A\r\33[2K";
        ...
        surface.saveToFile("Id" + std::to_string(i) + ".csv");
//    surface.saveToFile("TestOctree.csv");
  }
  return 0;
}