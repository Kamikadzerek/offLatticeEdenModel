...
class Surface// store all cells
{
  ...
  std::vector<Cell<T> *> getEdgeCells() {
    ...
    do {
      ...
      std::sort(adjacentCells.begin(), adjacentCells.end(), [](Cell<sf::CircleShape>::adjacentCell c1, Cell<sf::CircleShape>::adjacentCell c2) { return c1.angle < c2.angle; });
  ...
  std::vector<Cell<T> *> getEdgeCellsAltUpdate() {
    ...
    do {
      ...
      for (auto adjacentCell = adjacentCells.begin(); adjacentCell < adjacentCells.end(); adjacentCell++) {
        ...
        if (std::find_if(edgeCells.begin(), edgeCells.end(), [adjacentCell](Cell<T> *cell) { return adjacentCell->cell->getId() == cell->getId(); }) == edgeCells.end()) {
    ...
    for (int i = 0; i < numberOfIteration; i++) {
      ...
      if (iterationCounter % densityOfMeasurements == 0) {
        ...
        if (currentMeanRadius > lastMeanRadius) {
          ...
          //                    saveToFileMeanRadiusOfLivingCellsNumOfLivingCells("4MT.csv");
...
std::ostream &operator<<(std::ostream &out, const std::vector<typename Surface<T>::adjacentCell> &vecAC) {
  ...
  std::sort(vAC.begin(), vAC.end(), [](Surface<T>::adjacentCell c1, Surface<T>::adjacentCell c2) { return c1.angle < c2.angle; });
  out << "AdjacentCells:{";
